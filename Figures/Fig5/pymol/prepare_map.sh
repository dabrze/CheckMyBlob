...
for x in "1fpx_SAM_1699_A/1fpx" ; do
echo "Run $x"
pdb="$x.pdb"
mtz="$x"".mtz"
map="$x"
...
echo "$pdb $mtz $map"
fft hklin $mtz mapout $map'_2FoFc_org.map.ccp4' \
<<eof
title 2mFo-dFc map
labi F1=FWT PHI=PHWT
end
...
fft hklin $mtz mapout $map'_FoFc_org.map.ccp4' \
<<eof
title mFo-dFc map
labi F1=DELFWT PHI=PHDELWT
end 
...
mapmask mapin $map'_2FoFc_org.map.ccp4' xyzin $pdb mapout $map'_2FoFc_org.cover.ccp4' \
<<eof
BORDER 5.0
end 
eof

mapmask mapin $map'_FoFc_org.map.ccp4' xyzin $pdb mapout $map'_FoFc_org.cover.ccp4' \
<<eof
BORDER 5.0
end 
eof
